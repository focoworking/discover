...
- Motor maestro (v7): el estudio no se centra en la web. Tiene tres capas, que se consultan desde el servidor (`producto/funnel26/fuentes.php` → `_fuentes.php`):
  - OSM Overpass: la competencia y, en B2B, los clientes objetivo. La web, el teléfono y el horario son solo atributos.
  - Census: CBP 2022 (NAICS) y ACS 5 años. El Geocoder da el condado.
  - Estudio verificado FOCOw.
  - Cada concepto y sector del tesauro lleva `osm` y `cbp`; un nicho nuevo necesita ambos.
  - El paso 2 B2B pregunta «¿A qué tipo de empresas?» (`clientes`).
  - Caché en `../f26-datos/cache`: 30 días para el Census, 1 para OSM y 90 para el geocoder.
  - `?diag=fuentes` hace el diagnóstico en el deploy (solo avisa). `F26_MOCK_FUENTES` sirve para probar sin red.
  - Se atribuye a OSM (ODbL) y al Census.
  - Census: la API exige clave. Sin ella responde 302 hacia `missing_key.html`, tanto desde GoDaddy como desde GitHub Actions. Hace falta el secreto `CENSUS_API_KEY` en FOCO, que es gratis (api.census.gov/data/key_signup.html). Con él, `censo.mjs` genera en el deploy la instantánea de Florida (`_censo.php`) y el motor consulta en vivo fuera de FL (focoworking/FOCO#34).
  - Overpass: overpass-api.de a veces da 504; `f26_osm` pasa entonces a kumi.systems.
...
- v4.5 (2026-09-26): arreglado el tablero roto en producción por la inyección de GoDaddy dentro de la plantilla PDF (focoworking/FOCO#25–#27).
- v4.6 (2026-09-26): cabecera alta centrada, sin pestañas (focoworking/FOCO#28).
